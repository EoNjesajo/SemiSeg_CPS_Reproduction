...
class Cloud(BaseDataset):
    def __init__(self, setting, split_name, preprocess=None,
        ...
        super(Cloud, self).__init__(setting, split_name, preprocess, file_length)
    ...
    def __getitem__(self, index):
        ...
        if self.pseudo_label is True:
            img_path = os.path.join(self._img_path ,datadir, 'img', names + '.jpg')
            print(img_path)
        ...
        if 'test' in datadir:
            ...
            img_path = os.path.join(self._img_path ,datadir, 'img', sp[0] + '.jpg')

        if './' not in names:
            gt_path = os.path.join(self._gt_path , datadir, 'gt', sp[0] + '.png')
    ...
    def _fetch_data(self, img_path, gt_path=None, dtype=None):
        ...
        if gt_path is not None:
            ...
            gt = np.where(gt==76,1,gt)
            gt = np.where(gt==149,2,gt)
            gt = np.where(gt==225,3,gt)
            return img, gt
        return img, None

    @classmethod
    def get_class_colors(*args):
        return [[0,0,0], [234,51,35], [117,251,76],  [255,255,84]]

    @classmethod
    def get_class_names(*args):
        return ['background','thick', 'thin', 'shadow']
