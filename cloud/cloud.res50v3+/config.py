...
C.dataset_path = osp.join(C.volna, 'DATA/cloud')
...
''' Experiments Setting '''
C.train_source = osp.join(C.dataset_path, 'subset_train/train_labeled.txt')
C.unsup_source = osp.join(C.dataset_path, 'subset_train/train_unlabeled.txt')
...
''' Image Config '''
C.num_classes = 4
C.background = 0   # -1 : not included background
C.image_mean = np.array([0.485, 0.456, 0.406])  
C.image_std = np.array([0.229, 0.224, 0.225])
C.image_height = 512
C.image_width = 512

C.num_train_imgs = 163      # labeled samples
C.num_eval_imgs = 320
C.num_unsup_imgs = 2452     # unlabeled(unsupervised) samples
